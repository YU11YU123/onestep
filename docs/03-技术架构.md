...
| 表 | 关键字段 | 用途 |
|---|---|---|
| `tasks` | id、title、notes、status、bucket、planned_date、due_at、reminder_at、important、urgent、project_id | 任务主数据；important/urgent 在收集箱可为空，整理后为明确的是/否 |
...
2026-08-06 当前工作区已经包含 `onestep-app/` 浏览器原型和完整项目文档，不再是空目录。已知环境：Node `v24.18.0`、`npm.cmd 11.16.0` 可用；PowerShell 直接运行 `npm` 会被执行策略拦截，应使用 `npm.cmd`。2026-08-05 未找到 `rustc`、`cargo`、`cl`、`vswhere`；机器级注册表未发现 WebView2 项，但这不能证明系统完全未安装。

进入 Windows 桌面化前必须重新核验，并在获得用户许可后补齐 Rust、Windows C++ Build Tools 与 WebView2。不得把安装成功直接视为应用可构建，仍要实际完成一次开发构建和一次安装包构建。
